...
define(["require", "com/dalonedrow/engine/systems/base/interactive",
	...
	function InventoryData() {
		 ...
		 *//*
		this.ARX_INVENTORY_Declare_InventoryIn(
				final IO invOwnerIO,
		 ...
		 * @throws RPGException if an error occurs
		 *//*
		this.ARX_INVENTORY_IdentifyIO(final IO playerIO,
				final IO itemIO) throws RPGException {
			if (playerIO != null
		 ...
		 *//*
		this.ForcePlayerInventoryObjectLevel(final int level) {
			for (int i = numInventorySlots - 1; i >= 0; i--) {
				if (slots[i].getIo() != null) {
		 ...
		 * @throws RPGException if an error occurs
		 *//*
		this.ReplaceInAllInventories(final IO oldItemIO,
				final IO newItemIO)
						throws RPGException {
			if (oldItemIO != null
		 ...
		 * @throws RPGException if an error occurs
		 *//*
		this.SendInventoryObjectCommand(final String itemName,
